...
if __name__ == '__main__':
    ...
    ## rhs with error
    rhs_error = ca.vcat([v*np.cos(theta)+0.01, v*np.sin(theta)-0.03, omega])

    ## function
    f = ca.Function('f', [states, controls], [rhs], ['input_state', 'control_input'], ['rhs'])
    f_error = ca.Function('f_error', [states, controls], [rhs_error], ['input_state', 'control_input'], ['rhs'])
    ...
    while(np.linalg.norm(x0-xs)>1e-2 and mpciter-sim_time/T<0.0 ):
        ...
        u0 = estimated_opt[:N*2].reshape(N, n_controls) # (N, n_controls)
        x_m = estimated_opt[N*2:].reshape(N+1, n_states) # (N+1, n_states)
        x_c.append(x_m.T)
        u_c.append(u0[0, :])
        t_c.append(t0)
        t0, x0, u0, next_states = shift_movement(T, t0, x0, u0, x_m, f_error)
