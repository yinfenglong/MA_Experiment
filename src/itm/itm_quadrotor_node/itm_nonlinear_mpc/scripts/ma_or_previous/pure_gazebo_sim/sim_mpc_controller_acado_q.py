...
LastEditors: Wei Luo
LastEditTime: 2020-11-04 17:33:28
Note: Note
'''
...
class MPC_controller(object):
    def __init__(self, horizon=20,):
        ...
        self.att_command.type_mask = 0 # ignore rate
    ...
    def mpc_estimation_loop(self,):
        if self.trajectory_path is not None and self.current_state is not None:
            ...
            if not is_kf_success_:
                ...
                # print("kf result {0}".format(kf_estimated_))
                # ext_forces = np.tile(np.array([0.0, 0.0, 0.]), (self.time_horizon+1, 1))
            ...
            if self.enable_integrator:
                ...
                print(kf_estimated_forces_.shape)
                print(self.position_error_integration)
                print((np.array([0.2, 0.2, 0.3]).reshape(-1,1) * self.position_error_integration).shape)
            ...
            if not self.mpc_init:
                ...
                self.QN,
                ext_forces)
                acado_uav_rate.mpc_control_bounds(np.tile(np.array([-10.0/180.0*np.pi,-10.0/180.0*np.pi,-5.0/180.0*np.pi, 0.5*9.806]), (1, self.time_horizon)), np.tile(np.array([10.0/180.0*np.pi,10.0/180.0*np.pi,5.0/180.0*np.pi, 1.5*9.806]), (1, self.time_horizon)))
                self.mpc_init = True
            else:
