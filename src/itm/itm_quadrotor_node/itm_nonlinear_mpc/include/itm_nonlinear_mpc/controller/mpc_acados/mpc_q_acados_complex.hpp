 ...
 * @LastEditors: Wei Luo
 * @LastEditTime: 2021-08-19 15:13:59
 * @Note: Note
 */
#ifndef _MPC_ACADOS_Q_COMPLEX_HPP_
#define _MPC_ACADOS_Q_COMPLEX_HPP_
...
namespace acados_quadrotor
{
    ...
    private:
        ...
        bool is_velocity_progress_init_;
        double previous_time;
        double current_time;

        /* landing on a platform */
        bool has_landing_target;
        std::string landing_target_pose_topic;
        ros::Subscriber landing_target_pose_sub_;
        Eigen::Vector3d landing_target_2D_pose; // x, y, z
        ...
        /* get pid_thrust_ */
        void target_actual_control_callback(const mavros_msgs::ActuatorControlConstPtr &msg);

        /* velocity progress */
        void velocity_estimation(Eigen::Vector3d &vel);

        /* multi thread */
        boost::thread mpc_thread;
        void mpc_thread_function();

        /* landing target callback */
        void moving_target_pose_callback(const geometry_msgs::PoseStamped::ConstPtr &msg);
        void moving_target_odom_callback(const nav_msgs::Odometry::ConstPtr &msg);
    };
}
