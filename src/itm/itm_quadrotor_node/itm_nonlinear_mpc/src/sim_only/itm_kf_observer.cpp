      ...
      calibration_counter_(0)
{
  ...
  KF_pub_ = nh_.advertise<geometry_msgs::Pose>("/itm_quadrotor/KF",1000);
  initialize();
}
...
void KFDisturbanceObserver::initialize()
{
  ...
  ROS_INFO("start initializing itm_disturbance_observer:KF");

  // service_ = observer_nh_.advertiseService("StartCalibrateKF",
                                          //  &KFDisturbanceObserver::startCalibrationCallback, this);

  // observer_state_pub_ = observer_nh_.advertise<itm_nonlinear_mpc::itm_kf_observer>("observer_state", 10);

  // dynamic_reconfigure::Server<itm_nonlinear_mpc::KFDisturbanceObserverConfig>::CallbackType f;
...
void KFDisturbanceObserver::loadROSParams()
{
 ...
 if (!observer_nh_.getParam("r_attitude", r_attitude)) {
  ...
  ROS_INFO_STREAM("state_covariance_: \n" << state_covariance_);
    ...
    process_noise_covariance_(i) = q_position;
    process_noise_covariance_(i + 3) = q_velocity;
    process_noise_covariance_(i + 6) = q_attitude;
    process_noise_covariance_(i + 9) = q_angular_velocity;
    process_noise_covariance_(i + 12) = q_force;
    process_noise_covariance_(i + 15) = q_torque;

    measurement_covariance_(i) = r_position;
    measurement_covariance_(i + 3) = r_velocity;
    measurement_covariance_(i + 6) = r_attitude;
  }
...
//     itm_nonlinear_mpc::KFDisturbanceObserverConfig &config, uint32_t level)
// {
...
//   ROS_INFO("itm_nonlinear_mpc:KF dynamic config is called successfully");
...
void KFDisturbanceObserver::feedAttitudeCommand(const Eigen::Vector4d& roll_pitch_yaw_thrust_cmd)
{
  ...
  std::cout<<"measurements_"<<std::endl;
  std::cout<<this->measurements_<<std::endl;
}
...
bool KFDisturbanceObserver::updateEstimator()
{
  ...
  // std::cout<<state_covariance_<<std::endl;
  ...
  // ROS_INFO_STREAM("predicted_state_\n"<<predicted_state_);
  //  ROS_INFO_STREAM("measurements_\n"<<measurements_);
  ...
  geometry_msgs::Pose KF_pub_obj;
  KF_pub_obj.position.x = external_forces(0);
  KF_pub_obj.position.y =external_forces(1);
  KF_pub_obj.position.z =external_forces(2);
  // thrust_pub_obj.orientation.x = roll_ref;
  // thrust_pub_obj.orientation.y = pitch_ref;
  // thrust_pub_obj.orientation.z = yaw_rate_cmd;
  KF_pub_.publish(KF_pub_obj);
  ...
  // if (observer_state_pub_.getNumSubscribers() > 0) {
    // itm_nonlinear_mpc::itm_kf_observerPtr msg(new itm_nonlinear_mpc::itm_kf_observer);
...
void KFDisturbanceObserver::systemDynamics(double dt)
{
  ...
  std::cout<<"old state: "<<state_<<std::endl;
  ...
  std::cout<<predicted_state_<<std::endl;
