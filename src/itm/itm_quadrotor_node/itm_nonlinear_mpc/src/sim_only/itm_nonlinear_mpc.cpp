/*
name: itm_nonlinear_mpc.h
Yan Li, Uni Stuttgart, Germany
You can contact the author at <yan1.li@web.de>

NMPC: Non Linear Model Predictive Control
 */
#include <tf/transform_datatypes.h>
#include <itm_nonlinear_mpc/sim_only/itm_nonlinear_mpc.h>

namespace quadrotor_control
{   /* Instalieren */
    constexpr double NMPCQuadrotor::kGravity;
    constexpr int NMPCQuadrotor::kDisturbanceSize;
    NMPCQuadrotor::NMPCQuadrotor(const ros::NodeHandle& nh, const ros::NodeHandle& private_nh)
    :nh_(nh),
    private_nh_(private_nh),
    verbose_(true),
    initialized_parameters_(false),
    mpc_queue_(nh, private_nh, ACADO_N+1),
    position_error_integration_(0, 0, 0),
    command_roll_pitch_yaw_thrust_(0, 0, 0, 0),
    solve_time_average_(0),
    odometry_pose_Callback_flag_(false),
    UKF_observer_(nh, private_nh),
    KF_observer_(nh, private_nh),
    received_first_odometry_(false)
    {
        /* Instalieren */
        acado_initializeSolver();
        W_.setZero();
        WN_.setZero();
        input_.setZero();
        state_.setZero();
        reference_.setZero();
        referenceN_.setZero();

        /* Service serve: reset integrators*/
        reset_integrator_service_server_quadrotor_ = nh_.advertiseService(
          "reset_integrator", &NMPCQuadrotor::resetIntegratorServiceQuadrotorCallback, this);
        /* Publisher */
        thrust_pub_ = nh_.advertise<geometry_msgs::Pose>("/itm_quadrotor_control/nonlinear_mpc/command",1000);
         /* Subscriber current Odometry*/
        // odometry_pose_subscriber_ = nh_.subscribe<geometry_msgs::PoseStamped>("local_position_pose",10, &NMPCQuadrotor::OdometryPoseCallback, this);
        odometry_pose_subscriber_ = nh_.subscribe<nav_msgs::Odometry>("local_position_pose",10, &NMPCQuadrotor::OdometryPoseCallback, this);

        /* Subscriber UKF from a python node*/
        sub_ukf = nh_.subscribe("/estimated_pub", 1, &NMPCQuadrotor::pythonUKFCallback, this);
        ukf_available = false;

        initializeParameters();
        mpc_queue_.initializeQueue(sampling_time_, prediction_sampling_time_);

    }

    NMPCQuadrotor::~NMPCQuadrotor() //when new, means delete itself
    {
    }

    /* reset integrator service */
    bool NMPCQuadrotor::resetIntegratorServiceQuadrotorCallback(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
    {
        position_error_integration_.setZero();
        return true;
    }

    // void NMPCQuadrotor::OdometryPoseCallback(const nav_msgs::Odometry::ConstPtr& msg)
    // {
    //     Eigen::Vector3d position_W;
    //     Eigen::Quaterniond orientation_W_B;
    //     Eigen::Vector3d velocity_B;
    //     Eigen::Vector3d angular_velocity_B;

    //     position_W[0] = msg->pose.pose.position.x-0.12;
    //     position_W[1] = msg->pose.pose.position.y;
    //     position_W[2] = msg->pose.pose.position.z+0.045;

    //     orientation_W_B.x() = msg->pose.pose.orientation.x;
    //     orientation_W_B.y() = msg->pose.pose.orientation.y;
    //     orientation_W_B.z() = msg->pose.pose.orientation.z;
    //     orientation_W_B.w() = msg->pose.pose.orientation.w;

    //     Eigen::Vector3d velocity_world;
    //     velocity_world[0] = msg->twist.twist.linear.x;
    //     velocity_world[1] = msg->twist.twist.linear.y;
    //     velocity_world[2] = msg->twist.twist.linear.z;
    //     velocity_B = orientation_W_B.inverse() * velocity_world;

    //     angular_velocity_B[0] = msg->twist.twist.angular.x;
    //     angular_velocity_B[1] = msg->twist.twist.angular.y;
    //     angular_velocity_B[2] = msg->twist.twist.angular.z;

    //     /*set current_odometry_*/
    //     mav_msgs::EigenOdometry current_odometry_;
    //     current_odometry_.position_W = position_W;
    //     current_odometry_.orientation_W_B = orientation_W_B;
    //     current_odometry_.velocity_B = velocity_B;
    //     current_odometry_.angular_velocity_B = angular_velocity_B;
    //     setforOdometry(current_odometry_);
    //     odometry_pose_Callback_flag_ = true;

    // }

     void NMPCQuadrotor::OdometryPoseCallback(const nav_msgs::Odometry::ConstPtr& msg)
    {
         Eigen::Vector3d position_W;
        Eigen::Quaterniond orientation_W_B;
        Eigen::Vector3d velocity_B;
        Eigen::Vector3d angular_velocity_B;
        static Eigen::Vector3d previous_position_W;
        static Eigen::Quaterniond previous_orientation_W_B;

        static ros::Time t_previous = ros::Time::now();
        static bool do_once = true;
        double dt;
        if(do_once)
        {
            dt=0.04;
            previous_position_W = Eigen::Vector3d::Zero();
            previous_orientation_W_B = Eigen::Quaterniond::Identity();
            do_once=false;
        }else{
            ros::Time t0 = ros::Time::now();
            dt = (t0 - t_previous).toSec();
            t_previous = t0;
        }
        /*check that dt is not so different from 0.04*/
        if(dt > 0.1){
            dt = 0.1;
        }
        if(dt < 0.025){
            dt = 0.025;
        }

        position_W[0] = msg->pose.pose.position.x;
        position_W[1] = msg->pose.pose.position.y;
        position_W[2] = msg->pose.pose.position.z;

        orientation_W_B.x() = msg->pose.pose.orientation.x;
        orientation_W_B.y() = msg->pose.pose.orientation.y;
        orientation_W_B.z() = msg->pose.pose.orientation.z;
        orientation_W_B.w() = msg->pose.pose.orientation.w;

          /*get veloccity body*/
        Eigen::Vector3d velocity_world;
        velocity_world = (position_W-previous_position_W)/(dt+1e-7);
        velocity_B = orientation_W_B.inverse() * velocity_world;

        previous_position_W = position_W;
        /*get angular_velocity_B
        (q2 - q1) / dt = 0.5 * omega * q1
        omega = 2 * (q2 - q1) * conj( q1 ) / dt */
        Eigen::Quaterniond angular_velocity_B_Quaternion;
        angular_velocity_B_Quaternion.x() = (2 * (orientation_W_B.x() - previous_orientation_W_B.x())*(previous_orientation_W_B.conjugate().x()))/(dt+1e-7);
        angular_velocity_B_Quaternion.y() = (2 * (orientation_W_B.y() - previous_orientation_W_B.y())*(previous_orientation_W_B.conjugate().y()))/(dt+1e-7);
        angular_velocity_B_Quaternion.z() = (2 * (orientation_W_B.z() - previous_orientation_W_B.z())*(previous_orientation_W_B.conjugate().z()))/(dt+1e-7);
        angular_velocity_B_Quaternion.w() = (2 * (orientation_W_B.w() - previous_orientation_W_B.w())*(previous_orientation_W_B.conjugate().w()))/(dt+1e-7);

        mav_msgs::getEulerAnglesFromQuaternion(angular_velocity_B_Quaternion.normalized(),&angular_velocity_B);

        previous_orientation_W_B = orientation_W_B;


        /*set current_odometry_*/
        mav_msgs::EigenOdometry current_odometry_;
        current_odometry_.position_W = position_W;
        current_odometry_.orientation_W_B = orientation_W_B;
        current_odometry_.velocity_B = velocity_B;
        current_odometry_.angular_velocity_B = angular_velocity_B;
        setforOdometry(current_odometry_);
        odometry_pose_Callback_flag_ = true;

    }

     void NMPCQuadrotor::OdometryPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)

    {
        Eigen::Vector3d position_W;
        Eigen::Quaterniond orientation_W_B;
        Eigen::Vector3d velocity_B;
        Eigen::Vector3d angular_velocity_B;
        static Eigen::Vector3d previous_position_W;
        static Eigen::Quaterniond previous_orientation_W_B;

        static ros::Time t_previous = ros::Time::now();
        static bool do_once = true;
        double dt;
        if(do_once)
        {
            dt=0.04;
            previous_position_W = Eigen::Vector3d::Zero();
            previous_orientation_W_B = Eigen::Quaterniond::Identity();
            do_once=false;
        }else{
            ros::Time t0 = ros::Time::now();
            dt = (t0 - t_previous).toSec();
            t_previous = t0;
        }
        /*check that dt is not so different from 0.04*/
        if(dt > 0.1){
            dt = 0.1;
        }
        if(dt < 0.025){
            dt = 0.025;
        }

        position_W[0] = msg->pose.position.x;
        position_W[1] = msg->pose.position.y;
        position_W[2] = msg->pose.position.z;

        orientation_W_B.x() = msg->pose.orientation.x;
        orientation_W_B.y() = msg->pose.orientation.y;
        orientation_W_B.z() = msg->pose.orientation.z;
        orientation_W_B.w() = msg->pose.orientation.w;

        /*get veloccity body*/
        Eigen::Vector3d velocity_world;
        velocity_world = (position_W-previous_position_W)/(dt+1e-7);
        velocity_B = orientation_W_B.inverse() * velocity_world;

        previous_position_W = position_W;
        /*get angular_velocity_B
        (q2 - q1) / dt = 0.5 * omega * q1
        omega = 2 * (q2 - q1) * conj( q1 ) / dt */
        Eigen::Quaterniond angular_velocity_B_Quaternion;
        angular_velocity_B_Quaternion.x() = (2 * (orientation_W_B.x() - previous_orientation_W_B.x())*(previous_orientation_W_B.conjugate().x()))/(dt+1e-7);
        angular_velocity_B_Quaternion.y() = (2 * (orientation_W_B.y() - previous_orientation_W_B.y())*(previous_orientation_W_B.conjugate().y()))/(dt+1e-7);
        angular_velocity_B_Quaternion.z() = (2 * (orientation_W_B.z() - previous_orientation_W_B.z())*(previous_orientation_W_B.conjugate().z()))/(dt+1e-7);
        angular_velocity_B_Quaternion.w() = (2 * (orientation_W_B.w() - previous_orientation_W_B.w())*(previous_orientation_W_B.conjugate().w()))/(dt+1e-7);

        mav_msgs::getEulerAnglesFromQuaternion(angular_velocity_B_Quaternion.normalized(),&angular_velocity_B);

        previous_orientation_W_B = orientation_W_B;

        /*set current_odometry_*/
        mav_msgs::EigenOdometry current_odometry_;
        current_odometry_.position_W = position_W;
        current_odometry_.orientation_W_B = orientation_W_B;
        current_odometry_.velocity_B = velocity_B;
        current_odometry_.angular_velocity_B = angular_velocity_B;
        setforOdometry(current_odometry_);
        odometry_pose_Callback_flag_ = true;
    }//OdometryPoseCallback

    /*Initialize*/
    void NMPCQuadrotor::initializeParameters() {
        std::vector<double> drag_coefficients;

        /* Get parameters from RosParam server */
        private_nh_.param<bool>("verbose", verbose_, false);

        if (!private_nh_.getParam("mass", mass_))
        {
        ROS_ERROR("mass in nonlinear MPC controller is not loaded from ros parameter "
                "server");
        abort();
        }
        if (!private_nh_.getParam("roll_time_constant", roll_time_constant_)) {
        ROS_ERROR(
            "roll_time_constant in nonlinear MPC controller is not loaded from ros parameter server");
        abort();
        }
        if (!private_nh_.getParam("roll_gain", roll_gain_)) {
        ROS_ERROR("roll_gain in nonlinear MPC controller is not loaded from ros parameter server");
        abort();
        }
        if (!private_nh_.getParam("pitch_time_constant", pitch_time_constant_)) {
        ROS_ERROR(
            "pitch_time_constant in nonlinear MPC controller is not loaded from ros parameter server");
        abort();
        }
        if (!private_nh_.getParam("pitch_gain", pitch_gain_)) {
        ROS_ERROR("pitch_gain in nonlinear MPC controller is not loaded from ros parameter server");
        abort();
        }
        if (!private_nh_.getParam("linear_drag_coefficients", drag_coefficients)) {
        ROS_ERROR(
            "linear_drag_coefficients in nonlinear MPC controller is not loaded from ros parameter server");
        abort();
        }
        drag_coefficients_ << drag_coefficients.at(0), drag_coefficients.at(1), drag_coefficients.at(2);

        if (!private_nh_.getParam("antiwindup_ball", antiwindup_ball_)) {
        ROS_ERROR(
            "antiwindup_ball in nonlinear MPC controller is not loaded from ros parameter server");
        abort();
        }

        if (!private_nh_.getParam("position_error_integration_limit",
                                position_error_integration_limit_)) {
        ROS_ERROR(
            "position_error_integration_limit in nonlinear MPC is not loaded from ros parameter server");
        abort();
        }

        if (!private_nh_.getParam("enable_offset_free",
                                enable_offset_free_)) {
        ROS_ERROR(
            "enable_offset_free in nonlinear MPC is not loaded from ros parameter server");
        abort();
        }

        if (!private_nh_.getParam("sampling_time", sampling_time_)) {
        ROS_ERROR("sampling_time in nonlinear MPC is not loaded from ros parameter server");
        abort();
        }

        if (!private_nh_.getParam("prediction_sampling_time", prediction_sampling_time_)) {
        ROS_ERROR("prediction_sampling_time in nonlinear MPC is not loaded from ros parameter server");
        abort();
        }

        for (int i = 0; i < ACADO_N + 1; i++)
        {
        acado_online_data_.block(i, 0, 1, ACADO_NOD) << roll_time_constant_, roll_gain_, pitch_time_constant_, pitch_gain_, drag_coefficients_(
            0), drag_coefficients_(1), 0, 0, 0;
        }
        Eigen::Map<Eigen::Matrix<double, ACADO_NOD, ACADO_N + 1>>(const_cast<double*>(acadoVariables.od)) =
        acado_online_data_.transpose();

        ROS_INFO("acado!");

        if (verbose_)
        {
        std::cout << "acado online data: " << std::endl << acado_online_data_ << std::endl;
        }

        initialized_parameters_ = true;


        position_ref_.clear();
        velocity_ref_.clear();
        acceleration_ref_.clear();
        yaw_ref_.clear();
        yaw_rate_ref_.clear();

        ROS_INFO("Nonlinear MPC: initialized correctly");
    }//initializeforParameters()

    void NMPCQuadrotor::applyParameters()
    {
        W_.block(0, 0, 3, 3) = q_position_.asDiagonal();
        W_.block(3, 3, 3, 3) = q_velocity_.asDiagonal();
        W_.block(6, 6, 2, 2) = q_attitude_.asDiagonal();
        W_.block(8, 8, 3, 3) = r_command_.asDiagonal();

        WN_ = solveCARE((Eigen::VectorXd(6) << q_position_, q_velocity_).finished().asDiagonal(), r_command_.asDiagonal());
        Eigen::Map<Eigen::Matrix<double, ACADO_NY, ACADO_NY>>(const_cast<double*>(acadoVariables.W)) = W_
        .transpose();
        Eigen::Map<Eigen::Matrix<double, ACADO_NYN, ACADO_NYN>>(const_cast<double*>(acadoVariables.WN)) =
        WN_.transpose();

        for (size_t i = 0; i < ACADO_N; ++i)                   //N:Number of control/estimation intervals
        {
        acadoVariables.lbValues[3 * i] = -roll_limit_;       // min roll
        acadoVariables.lbValues[3 * i + 1] = -pitch_limit_;  // min pitch
        acadoVariables.lbValues[3 * i + 2] = thrust_min_;    // min thrust
        acadoVariables.ubValues[3 * i] = roll_limit_;        // max roll
        acadoVariables.ubValues[3 * i + 1] = pitch_limit_;   // max pitch
        acadoVariables.ubValues[3 * i + 2] = thrust_max_;    // max thrust
        }

        if (verbose_)
        {
        std::cout << "q_position_: " << q_position_.transpose() << std::endl;
        std::cout << "q_velocity_: " << q_velocity_.transpose() << std::endl;
        std::cout << "r_command_: " << r_command_.transpose() << std::endl;
        std::cout << "W_N = \n" << WN_ << std::endl;
        }
    }//NMPCQuadrotor::applyParameters()

    /* solve continuous time Riccati equation*/
    Eigen::MatrixXd NMPCQuadrotor::solveCARE(Eigen::MatrixXd Q, Eigen::MatrixXd R)
    {
        /*Define system matrices*/
        Eigen::MatrixXd A;
        A.resize(6, 6);
        A.setZero();
        A.block(0, 3, 3, 3) = Eigen::Matrix3d::Identity();
        A.block(3, 3, 3, 3) = -1.0 * drag_coefficients_.asDiagonal();

        Eigen::MatrixXd B;
        B.resize(6, 3);
        B.setZero();
        B(3, 1) = kGravity;
        B(4, 0) = -1.0 * kGravity;
        B(5, 2) = 1.0;

        Eigen::MatrixXd G = B * R.inverse() * B.transpose();

        Eigen::MatrixXd z11 = A;
        Eigen::MatrixXd z12 = -1.0 * G;
        Eigen::MatrixXd z21 = -1.0 * Q;
        Eigen::MatrixXd z22 = -1.0 * A.transpose();

        Eigen::MatrixXd Z;
        Z.resize(z11.rows() + z21.rows(), z11.cols() + z12.cols());
        Z << z11, z12, z21, z22;

        int n = A.cols();     //n=6
        Eigen::MatrixXd U(2 * n, 2 * n);  // 12x12 Orthogonal matrix from Schur decomposition
        Eigen::VectorXd WR(2 * n);      //12
        Eigen::VectorXd WI(2 * n);      //12
        lapack_int sdim = 0;  // Number of eigenvalues for which sort is true
        lapack_int info;      //lapack_int =int
        info = LAPACKE_dgees(LAPACK_COL_MAJOR,  // Eigen default storage order
            'V',               // Schur vectors are computed
            'S',               // Eigenvalues are sorted
            select_lhp,        // Ordering callback
            Z.rows(),          // Dimension of test matrix
            Z.data(),          // Pointer to first element
            Z.rows(),          // Leading dimension (column stride)
            &sdim,             // Number of eigenvalues sort is true
            WR.data(),         // Real portion of eigenvalues
            WI.data(),         // Complex portion of eigenvalues
            U.data(),          // Orthogonal transformation matrix
            Z.rows());         // Dimension of Z

        Eigen::MatrixXd U11 = U.block(0, 0, n, n).transpose();
        Eigen::MatrixXd U21 = U.block(n, 0, n, n).transpose();

        return U11.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(U21).transpose();

    }//solveCARE()

    /* set odom and commands */
    void NMPCQuadrotor::setforOdometry (const mav_msgs::EigenOdometry& odometry)
    {
        static mav_msgs::EigenOdometry previous_odometry = odometry;

        if (!received_first_odometry_)
        {
            /*get euler angles from quaternion*/
            Eigen::Vector3d euler_angles;
            odometry.getEulerAngles(&euler_angles);
            Eigen::VectorXd x0(ACADO_NX);
            x0 << odometry.getVelocityWorld(), euler_angles, odometry.position_W;
            initializeforAcadoSolver(x0);

            /*KF_observer_*/
            KF_observer_.reset(odometry.position_W, odometry.getVelocityWorld(), euler_angles,
                                odometry.angular_velocity_B, Eigen::Vector3d::Zero(),
                                Eigen::Vector3d::Zero());
            /*KF_observer_*/

            /*UKF_observer_*/
            // UKF_observer_.reset(odometry.position_W, odometry.getVelocityWorld(), euler_angles,
            //                     odometry.angular_velocity_B, Eigen::Vector3d::Zero(),
            //                     Eigen::Vector3d::Zero());
            /*UKF_observer_*/
            received_first_odometry_ = true;
        }

        if (odometry.position_W.allFinite() == false)
        {
            odometry_.position_W = previous_odometry.position_W;
            ROS_WARN("Odometry.position has a non finite element");
        } else {
            odometry_.position_W = odometry.position_W;
            previous_odometry.position_W = odometry.position_W;
        }

        if (odometry.velocity_B.allFinite() == false) {
            odometry_.velocity_B = previous_odometry.velocity_B;
            ROS_WARN("Odometry.velocity has a non finite element");
        } else {
            odometry_.velocity_B = odometry.velocity_B;
            previous_odometry.velocity_B = odometry.velocity_B;
        }

        if (odometry.angular_velocity_B.allFinite() == false) {
            odometry_.angular_velocity_B = previous_odometry.angular_velocity_B;
            ROS_WARN("Odometry.angular_velocity has a non finite element");
        } else {
            odometry_.angular_velocity_B = odometry.angular_velocity_B;
            previous_odometry.angular_velocity_B = odometry.angular_velocity_B;
        }

        odometry_.orientation_W_B = odometry.orientation_W_B;
        odometry_.timestamp_ns = odometry.timestamp_ns;
        previous_odometry.orientation_W_B = odometry.orientation_W_B;
        //  ROS_INFO_STREAM("odometry_.position_W:"<<odometry_.position_W.z());

    }//setforOdometry()

    void NMPCQuadrotor::setforCommandTrajectoryPoint(const mav_msgs::EigenTrajectoryPoint& command_trajectory)
    {
        //  ROS_INFO_STREAM("command_trajectory:"<<command_trajectory.position_W.z());
        mpc_queue_.insertReference(command_trajectory);
    }//setforCommandTrajectoryPoint()

    void NMPCQuadrotor::setforCommandTrajectory(const mav_msgs::EigenTrajectoryPointDeque& command_trajectory)
    {
        int array_size = command_trajectory.size();
        if (array_size < 2)
        {
            mav_msgs::EigenTrajectoryPoint command_trajectory_piont;
            command_trajectory_piont = command_trajectory.front();

            mpc_queue_.insertReference(command_trajectory_piont);
        }else{
            mpc_queue_.insertReferenceTrajectory(command_trajectory);
        }
        // mpc_queue_.insertReferenceTrajectory(command_trajectory);
    }//setforCommandTrajectory()

    /*initialize for AcadoSolver*/
    void NMPCQuadrotor::initializeforAcadoSolver(Eigen::VectorXd x0)
    {
        for (int i = 0; i < ACADO_N + 1; i++)
        {
            state_.block(i, 0, 1, ACADO_NX) << x0.transpose();
        }
        Eigen::Map<Eigen::Matrix<double, ACADO_NX, ACADO_N + 1>>(const_cast<double*>(acadoVariables.x)) = state_.transpose();//current state
        Eigen::Map<Eigen::Matrix<double, ACADO_NU, ACADO_N>>(const_cast<double*>(acadoVariables.u)) = input_.transpose(); // input
        Eigen::Map<Eigen::Matrix<double, ACADO_NY, ACADO_N>>(const_cast<double*>(acadoVariables.y)) = reference_.transpose();
        Eigen::Map<Eigen::Matrix<double, ACADO_NYN, 1>>(const_cast<double*>(acadoVariables.yN)) = referenceN_.transpose();
    }//initializeacadoforParameters()

    /*calculate RollPitchYawrateThrust Command*/
    bool NMPCQuadrotor::calculateRollPitchYawrateThrustCommand(Eigen::Vector4d* ref_attitude_thrust)
    {
        assert(ref_attitude_thrust != nullptr);
        assert(initialized_parameters_ == true);
        ros::WallTime starting_time = ros::WallTime::now();

        /*parameters estimated_disturbances, x_0 and current_rpy*/
        Eigen::VectorXd UKF_estimated_state;
        Eigen::VectorXd KF_estimated_state;
        Eigen::Vector3d estimated_disturbances;
        Eigen::Matrix<double, ACADO_NX, 1> x_0;

         /*get Euler Angles From Quaternion and save in address current_rpy*/
        Eigen::Vector3d current_rpy;
        odometry_.getEulerAngles(&current_rpy);//mav_control

        mpc_queue_.updateQueue();

        mpc_queue_.getQueue(position_ref_, velocity_ref_, acceleration_ref_, yaw_ref_, yaw_rate_ref_);

        /*UKF_observer_*/
        // UKF_observer_.feedAttitudeCommand(command_roll_pitch_yaw_thrust_);
        // UKF_observer_.feedPositionMeasurement(odometry_.position_W);
        // UKF_observer_.feedVelocityMeasurement(odometry_.getVelocityWorld());
        // UKF_observer_.feedRotationMatrix(odometry_.orientation_W_B.toRotationMatrix());

        // bool UKF_observer_update_successful = UKF_observer_.updateEstimator();
        // if (!UKF_observer_update_successful)
        // {
        //   UKF_observer_.reset(odometry_.position_W, odometry_.getVelocityWorld(), current_rpy,
        //                               Eigen::Vector3d::Zero(),Eigen::Vector3d::Zero(),
        //                               Eigen::Vector3d::Zero());
        // }
        // UKF_observer_.getEstimatedState(&UKF_estimated_state);

        // if (enable_offset_free_ == true) {
        //  estimated_disturbances = UKF_estimated_state.segment(12, kDisturbanceSize);
        // } else {
        //    estimated_disturbances.setZero(kDisturbanceSize);
        // }
        /*UKF_observer_*/


         /*UKF_observer.py*/
        // if (enable_offset_free_ == true && ukf_available) {
        //     estimated_disturbances = ukf_result;
        // } else {
        //     estimated_disturbances.setZero(kDisturbanceSize);
        // }
         /*UKF_observer.py*/

        /*KF_observer_*/
        KF_observer_.feedAttitudeCommand(command_roll_pitch_yaw_thrust_);
        KF_observer_.feedPositionMeasurement(odometry_.position_W);
        KF_observer_.feedVelocityMeasurement(odometry_.getVelocityWorld());
        KF_observer_.feedRotationMatrix(odometry_.orientation_W_B.toRotationMatrix());

        bool KF_observer_update_successful = KF_observer_.updateEstimator();
        if (!KF_observer_update_successful)
        {
            KF_observer_.reset(odometry_.position_W, odometry_.getVelocityWorld(), current_rpy,
                                        Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());
        }

        KF_observer_.getEstimatedState(&KF_estimated_state);

         if (enable_offset_free_ == true)
        {
            estimated_disturbances = KF_estimated_state.segment(12, kDisturbanceSize); //get external_forces
        } else {
            estimated_disturbances.setZero(kDisturbanceSize);
        }
        /*KF_observer_*/

        /*add integrator */
        if (enable_integrator_)
        {
            Eigen::Vector3d position_error = position_ref_.front() - odometry_.position_W;

            if (position_error.norm() < antiwindup_ball_)
            {
            position_error_integration_ += position_error * sampling_time_;
            }
            else
            {
            position_error_integration_.setZero();
            }

            position_error_integration_ = position_error_integration_.cwiseMax(
                Eigen::Vector3d(-position_error_integration_limit_, -position_error_integration_limit_,
                                -position_error_integration_limit_));

            position_error_integration_ = position_error_integration_.cwiseMin(
                Eigen::Vector3d(position_error_integration_limit_, position_error_integration_limit_,
                                position_error_integration_limit_));
            estimated_disturbances -= Eigen::Vector3d(Ki_xy_, Ki_xy_, Ki_altitude_).asDiagonal()
                * position_error_integration_;
        }


        double current_yaw = odometry_.getYaw();
        Eigen::Vector3d estimated_disturbances_B = odometry_.orientation_W_B.toRotationMatrix().transpose() * estimated_disturbances;

        for (size_t i = 0; i < ACADO_N; i++)
        {
            Eigen::Vector3d acceleration_ref_B = odometry_.orientation_W_B.toRotationMatrix().transpose()
                * acceleration_ref_[i];
            Eigen::Vector2d feed_forward(
                (-(acceleration_ref_B(1) - estimated_disturbances_B(1)) / kGravity),
                ((acceleration_ref_B(0) - estimated_disturbances_B(0)) / kGravity));
            reference_.block(i, 0, 1, ACADO_NY) << position_ref_[i].transpose(), velocity_ref_[i].transpose(),
            feed_forward.transpose(), feed_forward.transpose(), acceleration_ref_[i].z() - estimated_disturbances(2);


            acado_online_data_.block(i, ACADO_NOD - 3, 1, 3) << estimated_disturbances.transpose();  //??
        }
        /*terminal*/
        referenceN_ << position_ref_[ACADO_N].transpose(), velocity_ref_[ACADO_N].transpose();
        acado_online_data_.block(ACADO_N, ACADO_NOD - 3, 1, 3) << estimated_disturbances.transpose();

        x_0 << odometry_.getVelocityWorld(), current_rpy, odometry_.position_W;

        Eigen::Map<Eigen::Matrix<double, ACADO_NX, 1>>(const_cast<double*>(acadoVariables.x0)) = x_0;
        Eigen::Map<Eigen::Matrix<double, ACADO_NY, ACADO_N>>(const_cast<double*>(acadoVariables.y)) = reference_.transpose();
        Eigen::Map<Eigen::Matrix<double, ACADO_NYN, 1>>(const_cast<double*>(acadoVariables.yN)) = referenceN_.transpose();
        Eigen::Map<Eigen::Matrix<double, ACADO_NOD, ACADO_N + 1>>(const_cast<double*>(acadoVariables.od)) = acado_online_data_.transpose();
        // ROS_INFO_STREAM("acado_online_data_:\n"<<reference_.transpose());

        ros::WallTime time_before_solving = ros::WallTime::now();
        acado_preparationStep();
        int acado_status = acado_feedbackStep();

        solve_time_average_ += (ros::WallTime::now() - time_before_solving).toSec() * 1000.0;

        double roll_ref = acadoVariables.u[0];
        double pitch_ref = acadoVariables.u[1];
        double thrust_ref = acadoVariables.u[2];

        /*if (acado) solver failled, one service server writte to tell the service client,that MPC is succsseful or falled*/
        if (std::isnan(roll_ref) || std::isnan(pitch_ref) || std::isnan(thrust_ref) || acado_status != 0)
        {
            ROS_WARN_STREAM("Nonlinear MPC: Solver failed with status: " << acado_status);
            ROS_WARN("reinitializing...");
            initializeforAcadoSolver (x_0);
            *ref_attitude_thrust << 0, 0, 0, kGravity * mass_;
            return false;
        }

        command_roll_pitch_yaw_thrust_ << roll_ref, pitch_ref, yaw_ref_.front(), thrust_ref;
        state_ = Eigen::Map<Eigen::Matrix<double, ACADO_N + 1, ACADO_NX, Eigen::RowMajor>>(acadoVariables.x);

        /*yaw controller*/
        double yaw_error = yaw_ref_.front() - current_yaw;
        if (std::abs(yaw_error) > M_PI)
        {
            if (yaw_error > 0.0)
            {
            yaw_error = yaw_error - 2.0 * M_PI;
            } else
            {
            yaw_error = yaw_error + 2.0 * M_PI;
            }
        }
        double yaw_rate_cmd = K_yaw_ * yaw_error + yaw_rate_ref_.front();  // feed-forward yaw_rate cmd
        if (yaw_rate_cmd > yaw_rate_limit_)
        {
            yaw_rate_cmd = yaw_rate_limit_;
        }
        if (yaw_rate_cmd < -yaw_rate_limit_)
        {
            yaw_rate_cmd = -yaw_rate_limit_;
        }

        // *ref_attitude_thrust = Eigen::Vector4d(roll_ref, pitch_ref, yaw_rate_cmd, mass_ * thrust_ref);
        *ref_attitude_thrust = Eigen::Vector4d(roll_ref, pitch_ref, yaw_rate_cmd, thrust_ref);
        double diff_time = (ros::WallTime::now() - starting_time).toSec();

        /*Publischer command_roll_pitch_yaw_thrust_*/
        geometry_msgs::Pose thrust_pub_obj;
        thrust_pub_obj.position.x = 0.0;
        thrust_pub_obj.position.y = 0.0;
        thrust_pub_obj.position.z = thrust_ref;
        thrust_pub_obj.orientation.x = roll_ref;
        thrust_pub_obj.orientation.y = pitch_ref;
        thrust_pub_obj.orientation.z = yaw_rate_cmd;
        thrust_pub_.publish(thrust_pub_obj);


        if (verbose_)
        {
            static int counter = 0;
            if (counter > 100)
            {
                ROS_INFO_STREAM("average solve time: " << solve_time_average_ / counter << " ms");
                solve_time_average_ = 0.0;
                ROS_INFO_STREAM("Controller loop time : " << diff_time*1000.0 << " ms");
                ROS_INFO_STREAM("roll ref: " << command_roll_pitch_yaw_thrust_(0) << "\t" << "pitch ref : \t" << command_roll_pitch_yaw_thrust_(1) << "\t" << "yaw ref : \t" << command_roll_pitch_yaw_thrust_(2) << "\t" << "thrust ref : \t" << command_roll_pitch_yaw_thrust_(3) << "\t" << "yawrate ref : \t" << yaw_rate_cmd);
                counter = 0;
            }
            counter++;
        }

        return true;

    }//calculateRollPitchYawrateThrustCommand()

    /* get reference and predicted state*/
    bool NMPCQuadrotor::getforCurrentReference(mav_msgs::EigenTrajectoryPoint* reference)const
    {
        assert(reference != nullptr);
        (*reference).position_W = position_ref_.front();
        (*reference).velocity_W = velocity_ref_.front();
        (*reference).acceleration_W = acceleration_ref_.front();
        (*reference).setFromYaw(yaw_ref_.front());
        return true;
    }//getforCurrentReference()

    bool NMPCQuadrotor::getforCurrentReference(mav_msgs::EigenTrajectoryPointDeque* reference)const
    {
        assert(reference != nullptr);
        (*reference).clear();
        for (size_t i = 0; i < position_ref_.size(); i++)
        {
            mav_msgs::EigenTrajectoryPoint pnt;
            pnt.position_W = position_ref_.at(i);
            pnt.velocity_W = velocity_ref_.at(i);
            pnt.acceleration_W = acceleration_ref_.at(i);
            pnt.setFromYaw(yaw_ref_.at(i));
            (*reference).push_back(pnt);
        }
    return true;
    }//getforCurrentReference

    bool NMPCQuadrotor::getforPredictedState(mav_msgs::EigenTrajectoryPointDeque* predicted_state)const
    {
        assert(predicted_state != nullptr);

        for (size_t i = 0; i < ACADO_N + 1; i++)
        {
            mav_msgs::EigenTrajectoryPoint pnt;
            pnt.position_W = state_.block(i, 6, 1, 3).transpose();
            pnt.velocity_W = state_.block(i, 0, 1, 3).transpose();

            tf::Quaternion tf_orientation;
            tf_orientation.setRPY(state_(i,3), state_(i,4), state_(i,5));
            pnt.orientation_W_B.x() = tf_orientation.x();
            pnt.orientation_W_B.y() = tf_orientation.y();
            pnt.orientation_W_B.z() = tf_orientation.z();
            pnt.orientation_W_B.w() = tf_orientation.w();

            pnt.time_from_start_ns = static_cast<int64_t>(i) *
                                static_cast<int64_t>(sampling_time_ * 1000000000.0);
            pnt.timestamp_ns = odometry_.timestamp_ns + pnt.time_from_start_ns;
            (*predicted_state).push_back(pnt);
        }
        return true;
    }//getforPredictedState


    void NMPCQuadrotor::quaternion_cross(Eigen::Quaterniond q1, Eigen::Quaterniond q2, Eigen::Quaterniond &q_r){
        q_r.w() = q1.w()*q2.w() - (q1.x()*q2.x() + q1.y()*q2.y() + q1.z()*q2.z());
        q_r.x() = q1.w()*q2.x() + q2.w()*q1.x() - q1.y()*q2.z() + q1.z()*q2.y();
        q_r.y() = q1.w()*q2.y() + q1.y()*q2.w() + q1.x()*q2.z() - q1.z()*q2.x();
        q_r.z() = q1.w()*q2.z() + q1.z()*q2.w() + q1.y()*q2.x() - q1.x()*q2.y();
    }

    void NMPCQuadrotor::quaternion2vector(Eigen::Quaterniond q, Eigen::Vector3d &v_r){
        Eigen::Vector3d temp_v = q.vec();
        if (temp_v.norm() <= 1e-5)
        {
            v_r << 0.0, 0.0, 0.0;
        }
        else{
            v_r = 2.0*temp_v/temp_v.norm()*std::acos(q.w());
        }
    }

    void NMPCQuadrotor::vector2quaternion(Eigen::Vector3d v, Eigen::Quaterniond &q_r){
        if (v.norm()<=1e-5)
        {
            q_r.w() = 1.0;
            q_r.vec() = Eigen::Vector3d::Zero();
        }
        else{
            q_r.w() = std::cos(v.norm()/2.0);
            q_r.vec() = std::sin(v.norm()/2.0) * v / v.norm();
        }
    }

    void NMPCQuadrotor::quaternion_difference(Eigen::Quaterniond q1, Eigen::Quaterniond q2, Eigen::Quaterniond &q_r){
        q1.normalize();
        q2.normalize();
        quaternion_cross(q1, q2.conjugate(), q_r);
    }

    void NMPCQuadrotor::quaternion_difference(Eigen::Quaterniond q1, Eigen::Quaterniond q2, Eigen::Vector3d &v_r){
        Eigen::Quaterniond temp_q;
        q1.normalize();
        q2.normalize();

        quaternion_cross(q1, q2.conjugate(), temp_q);
        quaternion2vector(temp_q, v_r);
    }

    void NMPCQuadrotor::sum_quaternionAndvector(Eigen::Quaterniond q, Eigen::Vector3d v, Eigen::Quaterniond &q_r){
        Eigen::Quaterniond temp_q;
        vector2quaternion(v, temp_q);
        temp_q.normalize();
        q.normalize();
        quaternion_cross(temp_q, q.conjugate(), q_r);

    }

    void NMPCQuadrotor::pythonUKFCallback(const geometry_msgs::Point::ConstPtr& msg)
    {
        // ROS_ERROR("ukf");
        ukf_result[0]= msg->x;
        ukf_result[1]= msg->y;
        ukf_result[2]= msg->z;


        if(~ukf_available)
        ukf_available = true;
        ROS_INFO_STREAM("ukf"<<ukf_result);
    }

}//namespace quadrotor_control