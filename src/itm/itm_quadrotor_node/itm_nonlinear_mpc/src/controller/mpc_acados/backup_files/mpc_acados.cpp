 ...
 * @LastEditors: Wei Luo
 * @LastEditTime: 2021-07-14 14:25:11
 * @Note: Note
 */
...
namespace acados_quadrotor
{
                                                                                                             ...
                                                                                                             got_landing_pose(false)
    {
        ...
        pos_error_i_ = Eigen::Vector3d::Zero();
        last_pos_error_ = Eigen::Vector3d::Zero();
        rampup_active_ = true;
        rampup_init_ = false;
        thrust_cmd = 0.0;
        mpc_ready_ = false;
    ...
    void MPCAcadosController::takeoff_new()
    {
        for (int i = 0; i < time_horizon + 1; i++)
        {
            trajectory_reference(i, 0) = robot_init_state[0];
        ...
        {
            boost::shared_lock<boost::shared_mutex> mpc_lk(mutexMPCCallback_);
            pos_error_ << robot_init_state[0] - robot_current_state[0], robot_init_state[1] - robot_current_state[1], robot_init_state[2] + takeoff_height_ - robot_current_state[2];
            pos_error_i_ = pos_error_ + pos_error_i_;

            Eigen::Matrix3d R = robot_current_q.toRotationMatrix();
            Eigen::Vector3d pos_error_b_ = R * pos_error_;
            Eigen::Vector3d pos_error_i_b_ = R * pos_error_i_;
            Eigen::Vector3d pos_error_d_b_ = R * (pos_error_ - last_pos_error_);
            last_pos_error_ = pos_error_;
            if (pos_error_i_b_[2] > 0.2)
                pos_error_i_b_[2] = 0.2;
            else if (pos_error_i_b_[2] < -0.2)
                pos_error_i_b_[2] = -0.2;
            // std::cout << "pos error" << pos_error_ << "body frame error" << pos_error_b_ << std::endl;
            ...
            if (rampup_active_)
            {
                if (!rampup_init_)
                {
                    if (mpc_ready_)
                    {
                        ...
                        rampup_duration_ = control_acc_offset / 0.75;
                        rampup_init_ = true;
                        thrust_cmd = 0.0;
                    }
                }
                else
                {
                    if (fabs((ros::Time::now() - rampup_start_time_).toSec()) >= rampup_duration_)
                    {
                        ...
                        // thrust_cmd = pos_error_b_[2] * 0.6 + robot_command[2] / 9.8066 * control_acc_offset;
                    }
                    else
                    {
                        double rampup_dt = (ros::Time::now() - rampup_last_time_).toSec();
                        thrust_cmd += 0.75 * rampup_dt;
                        rampup_last_time_ = ros::Time::now();
                    }
                    std::cout << "thrust in rampup" << thrust_cmd << std::endl;
                }
            }
            else
                thrust_cmd = pos_error_b_[2] * 0.1 + robot_command[2] / 9.8066 * control_acc_offset;
            // thrust_cmd = (pos_error_b_[2] * .8 + pos_error_i_b_[2] * 0.2 + 0.7 * pos_error_d_b_[2] + 1.) * control_acc_offset;
            // max acceleration based on the constraint defined in MPC. One can edit it accordingly
            if (thrust_cmd >= 1.5 * control_acc_offset)
                thrust_cmd = 1.5 * control_acc_offset;
            // std::cout << "final output : " << thrust_cmd << std::endl;
            // std::cout << robot_command[2] << std::endl;
            msg.thrust = thrust_cmd;
            // msg.thrust = robot_command[3] / 9.8066 - (1 - control_acc_offset);
    ...
    void MPCAcadosController::robot_pose_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
    {
        ...
        robot_current_q = q_;
        Eigen::Vector3d rpy_;
        quaternion_to_rpy(robot_current_q, rpy_);
